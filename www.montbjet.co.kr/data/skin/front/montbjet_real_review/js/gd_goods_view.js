var gd_goods_view = function() {
    ...
    this.input_count_change = function(inputName, goodsFl) {
        ...
        if (nowCnt > maxCnt && maxCnt != 0 && maxCnt != '' && typeof maxCnt != 'undefined') {
            if(parseInt(maxCnt % salesUnit) > 0) {
				...
				_key = $(inputName).attr('data-key');
				$('input[data-key="'+_key+'"]').val($(inputName).data('value'));
                return false;
            } else {
                alert(setGoodsNm+__('최대수량은 %1$s이하입니다.', maxCnt));
                $(inputName).val($(inputName).data('value'));

				console.log($(inputName).attr('data-key'));
    ...
    this.option_price_display = function(inputName) {
        ...
        if ($(frmId + " input[name='selectGoodsFl']").length && $(frmId+" input[name='selectGoodsFl']").val()) {
			...
			$(".mbj_gdv_vv_r_wrap table.option_display_area tbody").remove();
        }
        ...
        if ($(frmId + " tr.optionKey"+setOptNo+"_" + arrTmp[0]).length && (setOptNo == "" ||  setOptNo == undefined) && setOptionTextFl !='y') {
			...
			_r_length = $(".mbj_gdv_vv_r_wrap table.option_display_area").length;

			if(_r_length > 0){
				$(".mbj_gdv_vv_r_wrap table.option_display_area"+setOptNo).append(addHtml);
			}else{
            ...
            if (setCartTabFl == 'y') {
				...
				$(".mbj_gdv_vv_r_wrap table.option_display_area").append(addHtml);
    ...
    this.remove_option = function(optionId) {
        ...
        $("#" + optionId).remove();
		$('.mbj_gdv_vv_r_wrap #' + optionId).remove(); 
    ...
    this.option_text_select = function(inputName) {
        ...
        var frmId = ".mbj_gdv_right #" + $(inputName).closest("form").attr('id');
		if($(inputName).closest("form").parent().parent().parent().hasClass('mbj_gdv_vv_r_wrap')){
			_this_name = $(inputName).attr('name');
			_this_val = $(inputName).val();
			$('.mbj_gdv_right input[name="'+_this_name+'"]').val(_this_val);
        ...
        if (displayOptionDisplay) {
            ...
            if ($(frmId + ' tr.optionKey'+setOptNo+'_' + tmpStr[0] + optionTextKey).length) {
				...
				$('.mbj_gdv_vv_r_wrap #option_text_display'+setOptNo+'_' + displayOptionkey).html(optionText);

                if(setOptNo != "" &&  setOptNo != undefined) {
                    $('span[id*="option_text_display' + setOptNo + '"]').html(optionText);
					$('.mbj_gdv_vv_r_wrap span[id*="option_text_display' + setOptNo + '"]').html(optionText);
                }else{
            ...
            if (displayOptionDisplay) {
                ...
                if ($(frmId + ' #add_goods_display_item_' + displayOptionkey + '_' + displayAddGoodsKey).length && (setOptNo == "" ||  setOptNo == undefined)) {
                    ...
                    if(setOptNo != "" && setOptNo != undefined){
						...
						$('.mbj_gdv_vv_r_wrap #option_display_item_' + displayOptionkey).append(addHtml);
                    }
    ...
    this.remove_add_goods= function(optionId,addGoodsId) {
		...
		$('.mbj_gdv_vv_r_wrap #add_goods_display_item_'+optionId+'_'+addGoodsId).remove();
    ...
    this.goods_calculate = function(frmId, goodsFl, itemNo, goodsCnt) {
        ...
        if (goodsFl) {
			...
			$('.mbj_gdv_vv_r_wrap .option_price_display_' + setOptNo + itemNo).html(gd_money_format(((optionTotalPrice + goodsPrice) * goodsCnt).toFixed(setDecimal)));
            ...
            if(setOptNo == "" || setOptNo == undefined) {
				...
				$('.mbj_gdv_vv_r_wrap .add_goods_price_display_' + itemNo + '_' + addGoodsItemNo).html(gd_money_format(addGoodsTotalPrice.toFixed(setDecimal)));
				...
				$('.mbj_gdv_vv_r_wrap .add_goods_price_display_' + setOptNo + '_' + itemNo + '_' + addGoodsItemNo).html(gd_money_format(addGoodsTotalPrice.toFixed(setDecimal)));
            }
